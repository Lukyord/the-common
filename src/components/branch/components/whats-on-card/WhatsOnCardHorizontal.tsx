...
export default function WhatsOnCardHorizontal({
  ...
  highlightText,
  link,
}: WhatsOnCardHorizontalProps) {
  return (
    <AnimateOnScroll
      triggerClass="fadeIn"
      data-card="whats-on"
      className={`card horizontal ${className}`}
      style={
        ...
        <div className="card-body">
          <div className="card-header">
            ...
            <div className="card-ttl">
              <MarkdownContent
                as="h3"
                className="type-d-title type-m-body-m letter-spacing-002 weight-medium"
              >
                {title}
              </MarkdownContent>
            </div>
          ...
          <div className="card-tags">
            {mainTag && (
              <Link
                href={`/whats-on/filter?branch=${branchSlug}&tag=${mainTag}`}
                className="tag main"
              >
