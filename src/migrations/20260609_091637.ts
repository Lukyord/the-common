...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`branch_vendor_pages\` (
  ...
  `)
  await db.run(
    sql`CREATE UNIQUE INDEX \`branch_vendor_pages_branch_idx\` ON \`branch_vendor_pages\` (\`branch_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_vendor_pages_meta_meta_image_idx\` ON \`branch_vendor_pages\` (\`meta_image_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_vendor_pages_updated_at_idx\` ON \`branch_vendor_pages\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_vendor_pages_created_at_idx\` ON \`branch_vendor_pages\` (\`created_at\`);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`branch_vendor_pages_id\` integer REFERENCES branch_vendor_pages(id);`,
  )
  await db.run(
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`branch_vendor_pages\`;`)
