...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`branches_id\` integer REFERENCES branches(id);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`branch_contact_pages_id\` integer REFERENCES branch_contact_pages(id);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`branch_space_rental_pages_id\` integer REFERENCES branch_space_rental_pages(id);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`blogs_id\` integer REFERENCES blogs(id);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`vendors_id\` integer REFERENCES vendors(id);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`whats_on_id\` integer REFERENCES whats_on(id);`,
  )
  await db.run(
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "lifestyle_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "lifestyle_id" FROM \`payload_locked_documents_rels\`;`,
  )
