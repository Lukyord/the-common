...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.run(
    sql`ALTER TABLE \`branch_space_rental_pages\` RENAME TO \`branch_venue_rental_pages\`;`,
  )
  await db.run(sql`CREATE TABLE \`venue_rental_page_branch_groups\` (
  	...
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`venue_rental_page\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_branch_groups_order_idx\` ON \`venue_rental_page_branch_groups\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_branch_groups_parent_id_idx\` ON \`venue_rental_page_branch_groups\` (\`_parent_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_branch_groups_branch_idx\` ON \`venue_rental_page_branch_groups\` (\`branch_id\`);`,
  )
  await db.run(sql`CREATE TABLE \`venue_rental_page\` (
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_meta_meta_image_idx\` ON \`venue_rental_page\` (\`meta_image_id\`);`,
  )
  await db.run(sql`CREATE TABLE \`venue_rental_page_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  	\`media_id\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`venue_rental_page\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`media_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_rels_order_idx\` ON \`venue_rental_page_rels\` (\`order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_rels_parent_idx\` ON \`venue_rental_page_rels\` (\`parent_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_rels_path_idx\` ON \`venue_rental_page_rels\` (\`path\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`venue_rental_page_rels_media_id_idx\` ON \`venue_rental_page_rels\` (\`media_id\`);`,
  )
  await db.run(sql`CREATE TABLE \`vendors_page\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text,
  	\`meta_title\` text,
  	\`meta_description\` text,
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`vendors_page_meta_meta_image_idx\` ON \`vendors_page\` (\`meta_image_id\`);`,
  )
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	\`branch_venue_rental_pages_id\` integer,
  	\`branch_vendor_pages_id\` integer,
  	\`branch_whats_on_pages_id\` integer,
  	\`blogs_id\` integer,
  	\`vendors_id\` integer,
  	\`whats_on_id\` integer,
  	...
  	FOREIGN KEY (\`branch_venue_rental_pages_id\`) REFERENCES \`branch_venue_rental_pages\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`branch_vendor_pages_id\`) REFERENCES \`branch_vendor_pages\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`branch_whats_on_pages_id\`) REFERENCES \`branch_whats_on_pages\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_branch_venue_rental_pages__idx\` ON \`payload_locked_documents_rels\` (\`branch_venue_rental_pages_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_branch_vendor_pages_id_idx\` ON \`payload_locked_documents_rels\` (\`branch_vendor_pages_id\`);`,
  )
  await db.run(
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`CREATE UNIQUE INDEX \`branch_space_rental_pages_branch_idx\` ON \`branch_space_rental_pages\` (\`branch_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_space_rental_pages_meta_meta_image_idx\` ON \`branch_space_rental_pages\` (\`meta_image_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_space_rental_pages_updated_at_idx\` ON \`branch_space_rental_pages\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`branch_space_rental_pages_created_at_idx\` ON \`branch_space_rental_pages\` (\`created_at\`);`,
  )
  await db.run(sql`DROP TABLE \`branch_venue_rental_pages\`;`)
  await db.run(sql`DROP TABLE \`venue_rental_page_branch_groups\`;`)
  await db.run(sql`DROP TABLE \`venue_rental_page\`;`)
  await db.run(sql`DROP TABLE \`venue_rental_page_rels\`;`)
  await db.run(sql`DROP TABLE \`vendors_page\`;`)
  await db.run(sql`PRAGMA foreign_keys=OFF;`)
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "branches_id", "branch_contact_pages_id", "branch_space_rental_pages_id", "branch_vendor_pages_id", "branch_whats_on_pages_id", "blogs_id", "vendors_id", "whats_on_id", "lifestyle_id", "vendor_categories_id", "whats_on_main_tags_id", "whats_on_sub_tags_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "branches_id", "branch_contact_pages_id", "branch_space_rental_pages_id", "branch_vendor_pages_id", "branch_whats_on_pages_id", "blogs_id", "vendors_id", "whats_on_id", "lifestyle_id", "vendor_categories_id", "whats_on_main_tags_id", "whats_on_sub_tags_id" FROM \`payload_locked_documents_rels\`;`,
  )
  await db.run(sql`DROP TABLE \`payload_locked_documents_rels\`;`)
  await db.run(
    sql`ALTER TABLE \`__new_payload_locked_documents_rels\` RENAME TO \`payload_locked_documents_rels\`;`,
